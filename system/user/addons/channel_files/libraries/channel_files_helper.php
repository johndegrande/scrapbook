...
class Channel_files_helper
{
	...
	{
		// Creat EE Instance
		ee()->load->library('firephp');

		$this->site_id = ee()->config->item('site_id');

		if (ee()->config->item('encryption_key') != FALSE) $this->ekey = ee()->config->item('encryption_key');
	}
	...
	function grab_settings($site_id=FALSE)
	{
		...
		if (isset(ee()->session->cache['channel_files_Settings']) == TRUE)
		{
			$settings = ee()->session->cache['channel_files_Settings'];
		}
		else
		{
			ee()->db->select('settings');
			ee()->db->where('module_name', 'channel_files');
			$query = ee()->db->get('exp_modules');
			if ($query->num_rows() > 0) $settings = unserialize($query->row('settings'));
		}

		ee()->session->cache['channel_files_Settings'] = $settings;
	...
	public function grab_field_settings($field_id)
	{
                ...
                $cache_settings = ee()->cache->get('Channel_Files/Field/'.$field_id);

                if (empty($cache_settings) === true)
		{
			$query = ee()->db->select('field_settings')->from('exp_channel_fields')->where('field_id', $field_id)->get();
                        ...
                        if (isset($settings['channel_files']) === true) {
				$settings = $settings['channel_files'];
			}
                        $settings = array_merge(ee()->config->item('cf_defaults'), $settings);

		//	if (isset($settings['channel_files']) == FALSE) echo "Limpiar"; $settings['channel_files'] = array();
			//$settings = $this->array_extend(ee()->config->item('cf_defaults'), $settings['channel_files']);
		}
		else
		{
			$settings =ee()->cache->get('Channel_Files/Field/'.$field_id);
		}

		// Any overrides?
		//$config = ee()->config->item('channel_files') ? ee()->config->item('channel_files') : array();
                if (isset($settings['settings']) == TRUE) {
                      $settings = array_merge($settings,  $settings['settings']['channel_files']);
                 }
		/*if (isset($config['fields'][$field_id]) === true) {
			$settings = $this->array_extend($settings, $config['fields'][$field_id]);
		}*/
                ee()->cache->save('Channel_Files/Field/'.$field_id,$settings,500);
		//ee()->session->cache['Channel_Files']['Field'][$field_id] = $settings;
//echo "DataBase2<pre>";
//           print_r($settings);
//               echo "<pre>";
		return $settings;
	...
	function get_upload_preferences($group_id = NULL, $id = NULL, $ignore_site_id = FALSE)
	{
		if (version_compare(APP_VER, '2.4', '>='))
		{
			...
			ee()->session->cache['upload_prefs'][$id] = $row;
			return $row;
		}
		...
		if ( ! empty($id))
		{
			...
			ee()->session->cache['upload_prefs'][$id] = $result;
			return $result;
		}
		...
		ee()->session->cache['upload_prefs'][$id] = $return_array;
	...
	function get_router_url($type='url', $method='channel_files_router')
	{
		...
		if (isset(ee()->session->cache['Channel_Files']['Router_Url'][$method]['ACT_ID']) == FALSE)
		{
			ee()->db->select('action_id');
			ee()->db->where('class', 'Channel_files');
			ee()->db->where('method', $method);
			$query = ee()->db->get('actions');
			$ACT_ID = $query->row('action_id');
		}
		else $ACT_ID = ee()->session->cache['Channel_Files']['Router_Url'][$method]['ACT_ID'];
		...
		if ($type == 'url')
		{
			...
			*/
			...
			if (isset(ee()->session->cache['Channel_Files']['Router_Url'][$method]['URL']) == TRUE) return ee()->session->cache['Channel_Files']['Router_Url'][$method]['URL'];
			ee()->session->cache['Channel_Files']['Router_Url'][$method]['URL'] = $ajax_url;
			return ee()->session->cache['Channel_Files']['Router_Url'][$method]['URL'];
		}
	...
	public function encrypt_string($string)
	{
		...
		if (function_exists('mcrypt_encrypt')) ee()->encrypt->set_cipher(MCRYPT_BLOWFISH);

		$string = ee()->encrypt->encode($string, substr(sha1(base64_encode($this->ekey)),0, 56));

		// Set it back
		if (function_exists('mcrypt_encrypt')) ee()->encrypt->set_cipher(MCRYPT_RIJNDAEL_256);
	...
	{
		ee()->load->library('encrypt');
		if (function_exists('mcrypt_decrypt')) ee()->encrypt->set_cipher(MCRYPT_BLOWFISH);

		$string = ee()->encrypt->decode($string, substr(sha1(base64_encode($this->ekey)),0, 56));
	...
	function mcp_meta_parser($type='js', $url, $name, $package='')
	{
		...
		if ($type == 'js')
		{
			if ( isset(ee()->session->cache['DevDemon']['JS'][$name]) == FALSE )
			{
				ee()->cp->add_to_foot('<script src="' . $url . '" type="text/javascript"></script>');
				ee()->session->cache['DevDemon']['JS'][$name] = TRUE;
			}
		}
		...
		if ($type == 'gjs')
		{
			if ( isset(ee()->session->cache['DevDemon']['GJS'][$name]) == FALSE )
			{
				...
				ee()->session->cache['DevDemon']['GJS'][$name] = TRUE;
			}
		}
	}
