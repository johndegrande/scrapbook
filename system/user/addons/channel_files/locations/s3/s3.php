	...
	private function init()
	{
		...
		else
		{
	        ...
	        if ($this->lsettings['storage'] == 'standard') {
	            $this->lsettings['storage'] = AmazonS3::STORAGE_STANDARD;
	        } else {
	            $this->lsettings['storage'] = AmazonS3::STORAGE_REDUCED;
	        }

			$temp = self::$module->get('s3_acl');
			$this->lsettings['acl'] = constant('AmazonS3::' . $temp[$this->lsettings['acl']]);

			$temp = self::$module->get('s3_regions');
