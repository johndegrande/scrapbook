...
class CF_Location_local extends Cfile_Location
{
	...
	public function get_location_prefs($location_id)
	{
		...
		if (isset(ee()->session->cache['upload_prefs'][$location_id]) === FALSE)
		{
			$location = ee()->channel_files_helper->get_upload_preferences(1, $location_id, TRUE);
		}
		else
		{
			$location = ee()->session->cache['upload_prefs'][$location_id];
		}
