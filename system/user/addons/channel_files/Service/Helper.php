...
class Helper
{
    protected $package_name = CHANNEL_FILES_CLASS_NAME;
    protected $package_version = CHANNEL_FILES_VERSION;
    ...
    public function getRouterUrl($type='url', $method='channel_files_router')
    {
        // -----------------------------------------
        // Grab action_id
        // -----------------------------------------
    ...
    public function mcpAssets($type, $name=null, $dir=null, $addon=false)
    {
        ...
        if ($type == 'gjs') {
            if (!ee()->session->cache($addon, 'gjs.' . $this->package_name)) {
                $UPLOAD_url = $this->getRouterUrl('url', 'upload_file');
                $SIMPLE_FILE_url = $this->getRouterUrl('url', 'simple_file_url');

                $js = " var ChannelFiles = ChannelFiles ? ChannelFiles : {};
                        ChannelFiles.AJAX_URL = '{$ajaxUrl}&site_id={$this->site_id}';
                        ChannelFiles.THEME_URL = '{$themeUrl}';
    ...
    function get_channel_id_from_param()
    {
        ...
        if ($param_channel != FALSE)
        {
            ...
            if (isset(ee()->session->cache['DevDemon']['AllChannelsLight']) == FALSE)
            {
                ...
                ee()->session->cache['DevDemon']['AllChannelsLight'] = $channels;
            }
            else
            {
                $channels = ee()->session->cache['DevDemon']['AllChannelsLight'];
            }
        }
        else
        {
...
/* Location: ./system/user/addons/channel_files/Service/Helper.php */