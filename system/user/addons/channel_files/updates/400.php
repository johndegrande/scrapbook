...
class ChannelFilesUpdate_400
{
	...
	public function do_update()
	{
		...
		// -----------------------------------------
		$query = ee()->db->query("SELECT field_id, field_settings FROM exp_channel_fields WHERE field_type = 'channel_files'");
		...
		foreach ($query->result() as $field)
		{
			...
			// Simple Settings
			// -----------------------------------------
			$settings = ee()->config->item('cf_defaults');
		...
		// -----------------------------------------
		if (ee()->db->field_exists('member_id', 'channel_files') == FALSE)
		{
			$fields = array( 'member_id'	=> array('type' => 'INT',	'unsigned' => TRUE, 'default' => 0) );
			ee()->dbforge->add_column('channel_files', $fields, 'channel_id');
		}

		// Grab all files!
		$query = ee()->db->select('cf.file_id, ct.author_id')->from('exp_channel_files cf')->join('exp_channel_titles ct', 'ct.entry_id = cf.entry_id', 'left')->get();

		foreach ($query->result() as $row)
		{
			ee()->db->where('file_id', $row->file_id);
			ee()->db->update('exp_channel_files', array('member_id' => $row->author_id));
		}

		$query->free_result();

		// Add cffield_1
		if (ee()->db->field_exists('cffield_1', 'channel_files') == FALSE)
		{
			$fields = array( 'cffield_1'=> array('type' => 'VARCHAR',	'constraint' => '250', 'default' => '') );
			ee()->dbforge->add_column('channel_files', $fields);
		}

		// Add cffield_2
		if (ee()->db->field_exists('cffield_2', 'channel_files') == FALSE)
		{
		...
		// Add cffield_3
		if (ee()->db->field_exists('cffield_3', 'channel_files') == FALSE)
		{
		...
		// Add cffield_4
		if (ee()->db->field_exists('cffield_4', 'channel_files') == FALSE)
		{
		...
		// Add cffield_5
		if (ee()->db->field_exists('cffield_5', 'channel_files') == FALSE)
		{
		...
		// Add URL TITLE
		if (ee()->db->field_exists('url_title', 'channel_files') == FALSE)
		{
			$fields = array( 'url_title'=> array('type' => 'VARCHAR',	'constraint' => '250', 'default' => '') );
			ee()->dbforge->add_column('channel_files', $fields, 'title');
		}

		// -----------------------------------------
		// Add new Action!
		// -----------------------------------------
		$query = ee()->db->query("SELECT action_id FROM exp_actions WHERE class = 'Channel_files' AND method = 'simple_file_url'");
		if ($query->num_rows() == 0)
		{
		...
		$query = ee()->db->query("SELECT action_id FROM exp_actions WHERE class = 'Channel_files' AND method = 'locked_file_url'");
		if ($query->num_rows() == 0)
		{
