...
class Channel_files_model
{
	...
	public function get_files($entry_id=FALSE, $field_id=FALSE, $params=array(), $tagdata='')
	{
		...
		{
			ee()->db->limit(1);
			ee()->db->order_by('file_primary DESC, file_order ASC');
		}
		elseif ( (isset($params['force_primary']) === TRUE && $params['force_primary'] == 'yes') )
		{
			ee()->db->where('file_primary', 1);
		}
		...
		foreach ($params as $key => $val) {
			...
			if (ctype_digit($num)) {
				ee()->db->where('cffield_' . $num, $member_id);
			}
		}
	...
	public function parse_template($entry_id=FALSE, $field_id=FALSE, $params=array(), $tagdata)
	{
		...
		if (ee()->TMPL->fetch_param('encrypt_locked_url') == 'no') ee()->session->cache['channel_files']['encrypt_locked_url'] = 'no';
		if (ee()->TMPL->fetch_param('simple_locked_url') == 'yes') ee()->session->cache['channel_files']['simple_locked_url'] = 'yes';
		...
		if (count($files) === 0)
		{
			ee()->TMPL->log_item("CHANNEL FILES: No files found.");
		...
		if (preg_match('/'.LD."{$this->prefix}paginate(.*?)".RD."(.+?)".LD.'\/'."{$this->prefix}paginate".RD."/s", $tagdata, $match))
		{
			...
			if ($this->total_files > $limit)
			{
				...
				$config['suffix']		= '';
				$config['first_link'] 	= ee()->lang->line('cf:pag_first_link');
				$config['last_link'] 	= ee()->lang->line('cf:pag_last_link');
		...
		if ( strpos($tagdata, $this->prefix.'locked_url') !== FALSE)
		{
			...
			// IP
			$this->IP = (isset($_SERVER['HTTP_X_FORWARDED_FOR']) == TRUE) ? $_SERVER['HTTP_X_FORWARDED_FOR'] : ee()->input->ip_address();

			// Grab Router URL
			$this->locked_act_url = ee()->channel_files_helper->get_router_url('url', 'locked_file_url');
		}
	...
	public function parse_single_file_row($count, $file, $tagdata)
	{
		...
		if ($this->locked_url == TRUE)
		{
			...
			$locked = array('time' => ee()->localize->now + 3600, 'file_id' => $file->file_id, 'ip' => $IP);
			if (isset(ee()->session->cache['channel_files']['simple_locked_url']) === TRUE && ee()->session->cache['channel_files']['simple_locked_url'] == 'yes')
			{
				$locked['simple'] = 'yes';
				unset($locked['time'], $locked['ip']);
			}

			if (isset(ee()->session->cache['channel_files']['encrypt_locked_url']) == TRUE && ee()->session->cache['channel_files']['encrypt_locked_url'] == 'no')
			{
				$vars[$this->prefix.'locked_url'] = $this->locked_act_url . '&amp;e=no&amp;key=' . base64_encode(serialize($locked));
			}
			else
			{
				$vars[$this->prefix.'locked_url'] = $this->locked_act_url . '&amp;key=' . base64_encode(ee()->channel_files_helper->encrypt_string(serialize($locked)));
			}
		}
	...
	public function get_location_prefs($location_id)
	{
		...
		if (isset(ee()->session->cache['Channel_Files']['Locations'][$location_id]) == FALSE)
		{
			$query = ee()->db->select('server_path, url')->from('exp_upload_prefs')->where('id', $location_id)->get();
			...
			ee()->session->cache['Channel_Files']['Locations'][$location_id] = $location;

			$query->free_result();
		}
		else
		{
			$location = ee()->session->cache['Channel_Files']['Locations'][$location_id];
		}
	...
	public function get_field_id($file)
	{
		...
		// Hard way
		if (isset(ee()->session->cache['Channel_Files']['Channel2Field'][$file->channel_id]) == FALSE)
		{
			...
			ee()->session->cache['Channel_Files']['Channel2Field'][$file->channel_id] = $query->row('field_id');
		...
		{
			$field_id = ee()->session->cache['Channel_Files']['Channel2Field'][$file->channel_id];
		}
	...
	public function get_channel_id($channels)
	{
		...
		if (strpos($channels, '|') !== FALSE)
		{
			...
			foreach ($channels as $key => $value)
			{
				...
				if (isset(ee()->session->cache['devdemon']['channel_to_id'][$value]) === TRUE)
				{
			...
			{
				$query = ee()->db->query("SELECT channel_id, channel_name FROM exp_channels WHERE channel_name IN (".implode(',', $lookup).") ");
		...
		{
			// Did we Cache this already?
			if (isset(ee()->session->cache['devdemon']['channel_to_id'][$channels]) === FALSE)
			{
				$query = ee()->db->query("SELECT channel_id FROM exp_channels WHERE channel_name = '{$channels}' ");
				if ($query->num_rows() == 0) return FALSE;

				ee()->session->cache['devdemon']['channel_to_id'][$channels] = $query->row('channel_id');
			}

			return ee()->session->cache['devdemon']['channel_to_id'][$channels];
		}
	}
	...
	public function get_fields_from_params($params)
	{
		...
		if (isset($params['field']) === TRUE)
		{
			...
			if (strpos($params['field'], '|') !== FALSE)
			{
				...
				{
					if (isset(ee()->session->cache['channel']['custom_channel_fields'][$this->site_id][ $field ]) === FALSE)
					{
						// Grab the field id
						$query = ee()->db->query("SELECT field_id FROM exp_channel_fields WHERE field_name = '{$field}' AND site_id = {$this->site_id} ");
						if ($query->num_rows() == 0)
						{
							if (isset(ee()->TMPL) === TRUE) ee()->TMPL->log_item('CHANNEL_FILES: Could not find field : ' . $field);
							return FALSE;
						}
						else
						{
							ee()->session->cache['channel']['custom_channel_fields'][$this->site_id][ $field ] = $query->row('field_id');
						}
					}

					$fields[] = ee()->session->cache['channel']['custom_channel_fields'][$this->site_id][ $field ];
				}
			}
			else
			{
				if (isset(ee()->session->cache['channel']['custom_channel_fields'][$this->site_id][ $params['field'] ]) === FALSE)
				{
					// Grab the field id
					$query = ee()->db->query("SELECT field_id FROM exp_channel_fields WHERE field_name = '{$params['field']}' AND site_id = {$this->site_id} ");
					if ($query->num_rows() == 0)
					{
						if (isset(ee()->TMPL) === TRUE) ee()->TMPL->log_item('CHANNEL_FILES: Could not find field : ' . $params['field']);
						return FALSE;
					}
					else
					{
						ee()->session->cache['channel']['custom_channel_fields'][$this->site_id][ $params['field'] ] = $query->row('field_id');
					}
				}

				return ee()->session->cache['channel']['custom_channel_fields'][$this->site_id][ $params['field'] ];
			}
		}
