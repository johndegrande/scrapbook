        ...
        <table cellspacing="0" cellpadding="0" class="CVTable"> <thead> <tr>
            ...
            <?php foreach ($vidSection as $video) :?>
                <tr class="ChannelVideosDrag" data='<?=base64_encode($this->channel_videos_helper->generate_json($video))?>'>
                    ...
                    <td><?=$video->title?><?php echo '<div style="border:1px solid #ccc;background-color:#ffffe6;padding:5px 10px;color:#000;">VARDUMP:<pre>';
                    var_dump($video);
                    echo '</pre></div>'; ?>
                    </td>
                    <td><?=$video->author?></td>
                    ...
                    <td><?=$video->views?></td>
                    <td><?=$this->channel_videos_helper->formatDate('%d-%M-%y %H:%i', $video->date)?> <?php echo $video->date; ?></td>
                </tr>
            <?php endforeach;?>
