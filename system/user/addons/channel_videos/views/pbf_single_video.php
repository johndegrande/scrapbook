<?php if (strtolower($layout) == 'table') : ?>
    <tr class="CVItem">
        <td> <a href="<?=$vid->video_url?>" class="PlayVideo"><img src="<?=$vid->video_img_url?>"></a></td>
        <td><?=$vid->video_title?></td>
        <td><?=$vid->video_author?></td>
        <td><?=sprintf("%0.2f", $vid->video_duration / 60)?> min</td>
        <td><?=$vid->video_views?></td>
        <td><?=$this->channel_videos_helper->formatDate('%d-%M-%y %H:%i', $vid->video_date)?> <?php echo $vid->video_date; ?></td>
        <td>
...
<?php else :?>
    <div class="CVItem VideoTile">
        <a href="<?=$vid->video_url?>" class="PlayVideo"><img src="<?=$vid->video_img_url?>"></a>
