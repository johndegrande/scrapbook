...
class Image_helper
{
    ...
    public function define_theme_url()
    {
        ...
        if (! defined('CHANNELIMAGES_THEME_URL')) {
            define('CHANNELIMAGES_THEME_URL', $theme_url . 'channel_images/');
        }
    ...
    public function get_router_url($type='url', $method='channel_images_router')
    {
        ...
        if (isset($this->EE->session->cache['Channel_Images']['Router_Url'][$method]['ACT_ID']) == false) {
            ...
            $ACT_ID = $this->EE->session->cache['Channel_Images']['Router_Url'][$method]['ACT_ID'];
        }
        ...
        if ($type == 'url') {
            ...
            if (isset($this->EE->session->cache['Channel_Images']['Router_Url'][$method]['URL']) == true) {
     ...
     * @return  bool
     */
    public static function is_natural_number($str)
    {
    ...
    public function get_channel_id_from_param()
    {
        ...
        if (strpos($param_channel_id, '|') !== false) {
            ...
            foreach ($temp as $item) {
                if ($this->is_natural_number($item) == false) {
                    continue;
                }
                $channel_id[] = $item;
            }
            ...
            if ($this->is_natural_number($param_channel_id) != false) {
    ...
    public function mcp_meta_parser($type, $url, $name, $package='')
    {
        // -----------------------------------------
        // CSS
        // -----------------------------------------
        if ($type == 'css') {
            if (isset($this->EE->session->cache['EEHarbor']['CSS'][$name]) == false) {
                ee()->cp->add_to_foot('<link rel="stylesheet" href="' . $url . '" type="text/css" media="print, projection, screen" />');
                $this->EE->session->cache['EEHarbor']['CSS'][$name] = true;
            }
        }
        ...
        // -----------------------------------------
        if ($type == 'js') {
            if (isset($this->EE->session->cache['EEHarbor']['JS'][$name]) == false) {
                ee()->cp->add_to_foot('<script src="' . $url . '" type="text/javascript"></script>');
                $this->EE->session->cache['EEHarbor']['JS'][$name] = true;
            }
        }
        ...
        // -----------------------------------------
        if ($type == 'gjs') {
            if (isset($this->EE->session->cache['EEHarbor']['GJS'][$name]) == false) {
                ...
                ee()->cp->add_to_foot('<script type="text/javascript">' . $js . '</script>');
                $this->EE->session->cache['EEHarbor']['GJS'][$name] = true;
            }
        }
    }
} // END CLASS
