...
class Channel_Images_AJAX
{
    ...
    public function upload_file()
    {
        ...
        if ($dbimage) {
            ...
            $key = (ee()->input->get_post('key'))?ee()->input->get_post('key'):time();
        }
        ...
        */
        ...
        foreach ($settings['action_groups'] as $group) {
            ...
            if (is_resource(ImageAction::$imageResource)) {
                imagedestroy(ImageAction::$imageResource);
            }
        }
        ...
        $image['url_title'] = ee('Format')->make('Text', $image['title'])->urlSlug();
        $image['description'] = '';
    ...
    private function replace_image($dbimage, $image, $settings, $temp_dir)
    {
        ...
        $data = array(
                        ...
                        'title' => $image['title'],
                        'url_title' => $API->process_field_string(ee('Format')->make('Text', $image['title'])->urlSlug()),
                        'filesize'  =>  $filesize,
                        'width'     =>  $width,
                        'height'    =>  $height,
                        'sizes_metadata' => $mt,
        ...
        $query = ee()->db->select('*')->from('exp_channel_images')->where('image_id', $dbimage->image_id)->get();
        ...
        $image_arr['small_img_url'] = $LOC->parse_image_url($entry_id, $new_filename);
        $image_arr['big_img_url'] = $LOC->parse_image_url($entry_id, $new_filename);

        $out = trim(json_encode($image_arr));
        exit($out);
    }
    ...
    public function delete_image()
    {
        ...
        if (ee('Request')->post('field_id') == false) {
        ...
        foreach ($settings['action_groups'] as $group) {
            ...
            if ($entry_id > 0) {
                $res = $LOC->delete_file($entry_id, $name);
            } else {
                @unlink($this->moduleSettings['cache_path'].'channel_images/'.$key.'/'.$name);
            }
        }
        ...
        if ($entry_id > 0) {
            $res = $LOC->delete_file($entry_id, $filename);
        } else {
            @unlink($this->moduleSettings['cache_path'].'channel_images/'.$key.'/'.$filename);
        }
    ...
    public function apply_action()
    {
        ...
        // -----------------------------------------
        if ($stage == 'save') {
            if (file_exists($temp_dir.$filename) == false) {
                exit('ERROR: MISSING PREVIEW IMAGE FILE');
            }
        ...
        // -----------------------------------------
        $actions = ee('channel_images:Actions')->actions;
        if (isset($_POST['channel_images'][$action]) == false) {
            $action_settings = array();
        } else {
            $action_settings = $_POST['channel_images'][$action];
        }
    ...
    public function load_entries()
    {
        ...
        if ($filter == lang('ci:filter_keywords')) {
            $filter = false;
        }
        ...
        if ($field_id == false) {
            exit('MISSING FIELD ID');
        }

        // Get Field --> Old EE3 code for getting channel_id
        ...
        // Get Channels
        $channels = array();
        $query = ee()->db->query("SELECT channel_id FROM exp_channels_channel_fields WHERE field_id = {$field_id}");
        foreach ($query->result() as $row) {
            $channels[] = $row->channel_id;
        }

        $query1 = ee()->db->query("SELECT group_id FROM exp_channel_field_groups_fields WHERE field_id = {$field_id} LIMIT 1");
        if ($query1->num_rows() > 0) {
            $field_group_id = $query1->row('group_id');
            $query1 = ee()->db->query("SELECT channel_id FROM exp_channels_channel_field_groups WHERE group_id = {$field_group_id}");
            foreach ($query1->result() as $row2) {
                $channels[] = $row2->channel_id;
            }
        }
        // Get entries
        ...
        if ($filter) {
            ee()->db->like('title', $API->process_field_string($filter), 'both');
        }
        ee()->db->where('status !=', 'closed');
        ee()->db->where('entry_id !=', $entry_id);
        if (isset($settings['stored_images_by_author']) == true && $settings['stored_images_by_author'] == 'yes') {
    ...
    public function load_images()
    {
        ...
        if ($entry_id != false) {
            $images->filter('entry_id', $entry_id);
        }
        ...
        if ($title != false && $title != $settings['columns']['title']) {
            ee()->db->like('title', $title, 'both');
        }
        if ($desc != false && $desc != $settings['columns']['desc']) {
            ee()->db->like('description', $desc, 'both');
        }
        if ($category != false && $category != $settings['columns']['category']) {
            ee()->db->like('category', $category, 'both');
        }
        if ($cifield_1 != false && $cifield_1 != $settings['columns']['cifield_1']) {
            ee()->db->like('cifield_1', $cifield_1, 'both');
        }
        if ($cifield_2 != false && $cifield_2 != $settings['columns']['cifield_2']) {
            ee()->db->like('cifield_2', $cifield_2, 'both');
        }
        if ($cifield_3 != false && $cifield_3 != $settings['columns']['cifield_3']) {
            ee()->db->like('cifield_3', $cifield_3, 'both');
        }
        if ($cifield_4 != false && $cifield_4 != $settings['columns']['cifield_4']) {
            ee()->db->like('cifield_4', $cifield_4, 'both');
        }
        if ($cifield_5 != false && $cifield_5 != $settings['columns']['cifield_5']) {
            ee()->db->like('cifield_5', $cifield_5, 'both');
        }
        ...
        if ($images->count() == 0) {
            exit('<div><p>' . lang('ci:no_images') . '</p></div>');
        }
    ...
    public function add_linked_image()
    {
        ...
        // -----------------------------------------
        if (isset($settings['small_preview']) == false or $settings['small_preview'] == false) {
            if (isset($settings['action_groups'][1]['group_name'])) {
                $settings['small_preview'] = $settings['action_groups'][1]['group_name'];
            }
        }
    ...
    public function grab_image_totals()
    {
        ...
        $channels = array();
        $query = ee()->db->select('channel_id')->from('exp_channels_channel_fields')->where('field_id', $field_id)->get();
    ...
    public function import_files_ui()
    {
        ...
        if ($remaining < 1) {
            exit(lang('ci:import:remain_limit'));
        }
        ...
        // Double check
        if ($settings['show_import_files'] != 'yes') {
            exit('IMPORT FILES IS DISABLED');
        }

        // Check the path
        if (@is_dir($settings['import_path']) == false) {
            exit(lang('ci:import:bad_path'));
        }
        ...
        foreach ($dirfiles as $file) {
            ...
            if (is_dir($settings['import_path'].$file) === true) {
                continue;
            }
            if ($remaining == 0) {
                break;
            ...
            if (is_array($settings['file_extensions']) && in_array($extension, $settings['file_extensions']) != true) {
                continue;
            }
    ...
    public function import_images()
    {
        ...
        if (isset($_POST['files']) == false or empty($_POST['files']) == true) {
            exit(json_encode($out));
        }
    ...
    public function edit_image_ui()
    {
        ...
        $data['sizes'] = array();
        foreach ($settings['action_groups'] as $group) {
            if (isset($group['editable']) == false or $group['editable'] == 'no') {
                continue;
            }
    ...
    public function apply_edit_image_action()
    {
        ...
        if ($action == 'rotate-left' or $action == 'rotate-right') {
            if ($action == 'rotate-right') {
                $actions['rotate']->settings = array('degrees' => 90, 'background_color' => 'ffffff');
            } else {
                $actions['rotate']->settings = array('degrees' => -90, 'background_color' => 'ffffff');
            }
        ...
        if ($action == 'flip-hor' or $action == 'flip-ver') {
            if ($action == 'flip-ver') {
                $actions['flip']->settings = array('axis' => 'vertical');
            } else {
                $actions['flip']->settings = array('axis' => 'horizontal');
            }
    ...
    public function import_matrix_images()
    {
        ...
        if ($field_type == 'file') {
            $entry = ee('Model')->get('ChannelEntry', $entry_id)
                                    ...
                                    ->first();
            $query = $entry->getValues();

            $field_data = $query['field_id_'.$field_id];
        }
            ...
            foreach ($matrixData as $entry_id => $rows) {
                ...
                foreach ($rows as $count => $row) {
                    ...
                    foreach ($cols as $col_id => $map) {
                        ...
                        if (isset($row['col_id_'.$col_id]) === true) {
                            $img['map'][$map] = $row['col_id_'.$col_id];
                        }
                    }
        ...
        foreach ($images as $count => $img) {
            $image = array();
            $image['site_id']   =  ee()->config->item('site_id');
    ...
    public function display_replace_image_ui()
    {
        ...
        $data['temp_img_id'] = ee()->input->get_post('temp_img_id');
        $data['field_id'] = ee()->input->get_post('field_id');
        //$data['key'] = ee()->input->get_post('key');
    ...
    public function get_image_details()
    {
        ...
        if ($query->num_rows() == 0) {
            ...
            $out['field'] = $query->row('field_label');
        }
    ...
    public function get_image_sizes()
    {
        ...
        foreach ($ids as $key => $id) {
            ...
            ee()->db->select('field_id, field_settings, field_label');
            ee()->db->from('exp_channel_fields');
            ee()->db->where('field_id', $id);
            $query = ee()->db->get();

            ee()->db->select('cfg.group_id, fg.group_name');
            ee()->db->from('exp_channel_field_groups_fields cfg');
            ee()->db->where('cfg.field_id', $id);
            ee()->db->join('exp_field_groups fg', 'fg.group_id = cfg.group_id', 'left');
            $querygroup = ee()->db->get();

            $settings = @unserialize(base64_decode($query->row('field_settings')));

            if (isset($settings['channel_images']['action_groups']) == false) {
                continue;
            }
            ...
            $out['fields'][] = array(
                'group' => $querygroup->row('group_name'),
