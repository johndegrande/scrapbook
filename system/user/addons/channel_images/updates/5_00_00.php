...
class ChannelImagesUpdate_50000
{
    ...
    public function do_update()
    {
        ...
        foreach ($query->result() as $field) {
            ...
            // Simple check
            if (isset($oldsettings['channel_images']) == true) {
                continue;
            }
            ...
            // Location ID
            if (isset($oldsettings['ci_upload_location'])) {
                $settings['locations']['local']['location'] = $oldsettings['ci_upload_location'];
            }
            unset($oldsettings['ci_upload_location']);

            // Categories
            if (isset($oldsettings['ci_categories'])) {
                $settings['categories'] = $oldsettings['ci_categories'];
            }
            unset($oldsettings['ci_categories']);

            // Keep Original
            if (isset($oldsettings['ci_keep_original'])) {
                $settings['keep_original'] = $oldsettings['ci_keep_original'];
            }
            unset($oldsettings['ci_keep_original']);

            // Columns
            if (isset($oldsettings['ci_columns'])) {
                $settings['columns'] = $oldsettings['ci_columns'];
            }
            unset($oldsettings['ci_columns']);

            // Show Stored Images
            if (isset($oldsettings['ci_show_stored_images'])) {
                $settings['show_stored_images'] = $oldsettings['ci_show_stored_images'];
            }
            unset($oldsettings['ci_show_stored_images']);

            // Stored Image by Author
            if (isset($oldsettings['ci_stored_images_by_author'])) {
                $settings['stored_images_by_author'] = $oldsettings['ci_stored_images_by_author'];
            }
            unset($oldsettings['ci_stored_images_by_author']);

            // Image Limit
            if (isset($oldsettings['ci_image_limit'])) {
                $settings['image_limit'] = $oldsettings['ci_image_limit'];
            }
            unset($oldsettings['ci_image_limit']);

            // Allow Upsizing?
            $upsize = 'no';
            if (isset($oldsettings['ci_allow_upsizing']) && $oldsettings['ci_allow_upsizing'] == 'yes') {
                $upsize = 'yes';
            }
            ...
            if (isset($oldsettings['ci_image_sizes'])) {
                ...
                foreach ($oldsettings['ci_image_sizes'] as $size_name => $options) {
                    ...
                    if (isset($options['watermark']) == true && $options['watermark'] == 'y' && isset($oldsettings['ci_watermark']) == true && $oldsettings['ci_watermark']['type'] != 'none') {
                        ...
                        } elseif ($oldsettings['ci_watermark']['type'] == 'image') {
