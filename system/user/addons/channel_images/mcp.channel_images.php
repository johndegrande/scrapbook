...
use EEHarbor\ChannelImages\FluxCapacitor\Base\Mcp;
 ...
 */
class Channel_images_mcp extends Mcp
{
    /**
     * Views Data
     * @var array
    ...
    public function __construct()
    {
        ...
        //ee('channel_images:Helper')->mcpAssets('js', 'handlebars.min.js');
    ...
    public function batch_actions()
    {
        ...
        ee()->db->select('cf.field_id, cf.field_label, ff.group_name');
        ee()->db->from('exp_channel_fields cf');
        ee()->db->where('cf.site_id', $this->site_id);
        ee()->db->where('cf.field_type', 'channel_images');
        ee()->db->join('exp_channels_channel_fields ct', 'ct.field_id = cf.field_id', 'left');
        ee()->db->join('exp_channels_channel_field_groups fg', 'fg.channel_id = ct.channel_id', 'left');
        ee()->db->join('exp_field_groups ff', 'ff.group_id = fg.group_id', 'left');
        $query = ee()->db->get();
    ...
    public function import()
    {
        ...
        ee()->cp->load_package_css('channel_images_cp');
        ...
        // -----------------------------------------
        ee()->db->select('cf.field_label, cf.field_type, cf.field_id, ff.group_name');
        ee()->db->from('exp_channel_fields cf');
        ee()->db->join('exp_channels_channel_fields ct', 'ct.field_id = cf.field_id', 'left');
        ee()->db->join('exp_channels_channel_field_groups fg', 'ct.channel_id = fg.channel_id', 'left');
        ee()->db->join('exp_field_groups ff', 'ff.group_id = fg.group_id', 'left');
        ...
        foreach ($query->result() as $row) {
            ...
            $q2 = ee()->db->select('field_id, field_label')->from('exp_channel_fields')->where('site_id', $this->site_id)->where('field_type', 'channel_images')->get();
            ...
            if ($row->field_type == 'file') {
                // Grab all entry ids
                $q5 = ee()->db->select('entry_id')->from('exp_channel_data_field_'.$row->field_id.' ')->where('field_id_'.$row->field_id. ' !=', '')->get();
            }

            // Grab channel id's
            $q4 = ee()->db->select('channel_id')->from('exp_channels_channel_fields')->where('field_id', $row->field_id)->get();
