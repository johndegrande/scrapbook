...
namespace EEHarbor\ChannelImages\Model;
    ...
    public function getFieldId($image=null)
    {
        ...
        if (isset(self::$_cachedChannelToField[$image->channel_id]) == false) {
            ...
            $query = ee()->db->query("SELECT cf.field_id FROM exp_channel_fields AS cf
                                            LEFT JOIN exp_channels_channel_fields AS ct ON ct.field_id = cf.field_id
                                            WHERE ct.channel_id = '{$image->channel_id}' AND cf.field_type = 'channel_images'");
            if ($query->num_rows() == 0) {
                $query->free_result();
                return 0;
            }
