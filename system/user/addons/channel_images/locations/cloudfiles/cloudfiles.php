...
class CI_Location_cloudfiles extends Image_Location
{
    ...
    public function parse_image_url($dir, $filename)
    {
        ...
        if ($cdn_uri) {
            ...
            $first_url = $this->CF_CONT->cdn_uri;
        }
    ...
    public function test_location()
    {
        ...
        if ($safemode == 'on' || $safemode == 'yes' || $safemode == 'true' ||  $safemode == 1) {
            $o .= '<li class="remove">PHP Safe Mode (OFF): <span class="bad">Failed</span></li>';
        } else {
            $o .= '<li class="act">PHP Safe Mode (OFF): <span class="good">Passed</span></li>';
        }
    ...
    private function init()
    {
        if (isset($this->CF_CONN) == true) {
            ...
            // Which Region?
            if ($this->lsettings['region'] == 'uk') {
                $this->lsettings['region'] = constant('UK_AUTHURL');
            } else {
                $this->lsettings['region'] = constant('US_AUTHURL');
            }
