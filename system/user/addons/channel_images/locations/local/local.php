...
{
    protected static $cachedUploadDest = array();
    ...
    public function test_location()
    {
        ...
        if ($safemode == 'on' || $safemode == 'yes' || $safemode == 'true' ||  $safemode == 1) {
            $o .= '<li class="remove">PHP Safe Mode (OFF): <span style="color:red">Failed</span></li>';
        } else {
            $o .= '<li class="act">PHP Safe Mode (OFF): <span style="color:green">Passed</span></li>';
        }

        // Is DIR?
        if (is_dir($dir) === true) {
            $o .= '<li class="act">Is Dir: <span style="color:green">Passed</span></li>';
        } else {
            $o .= '<li class="remove">Is Dir: <span style="color:red">Failed</span></li>';
        }

        // Is READABLE?
        if (is_readable($dir) === true) {
            $o .= '<li class="act">Is Readable: <span style="color:green">Passed</span></li>';
        } else {
            $o .= '<li class="remove">Is Readable: Failed</li>';
        }

        // Is WRITABLE
        if (is_writable($dir) === true) {
            $o .= '<li class="act">Is Writable: <span style="color:green">Passed</span></li>';
        } else {
            $o .= '<li class="remove">Is Writable: <span style="color:red">Failed</span></li>';
        }

        // CREATE TEST FILE
        $file = uniqid(mt_rand()).'.tmp';
        if (@touch($dir.$file) === true) {
            $o .= '<li class="act">Create Test File: <span style="color:green">Passed</span></li>';
        } else {
            $o .= '<li class="remove">Create Test File: <span style="color:red">Failed</span></li>';
        }

        // DELETE TEST FILE
        if (@unlink($dir.$file) === true) {
            $o .= '<li class="act">Delete Test File: <span style="color:green">Passed</span></li>';
        } else {
            $o .= '<li class="remove">Delete Test File: <span style="color:red">Failed</span></li>';
        }
        ...
        if (@mkdir($dir.$tempdir, 0777, true) === true) {
            ...
            $o .= '<li class="act">Create Test DIR: <span style="color:red">Failed</span></li>';
        }


        // RENAME TEST DIR
        if (@rename($dir.$tempdir, $dir.$tempdir.'temp') === true) {
            $o .= '<li class="act">Rename Test DIR: <span style="color:green">Passed</span></li>';
        } else {
            $o .= '<li class="remove">Rename Test DIR: <span style="color:red">Failed</span></li>';
        }

        // DELETE TEST DIR
        if (@rmdir($dir.$tempdir.'temp') === true) {
            $o .= '<li class="act">Delete Test DIR: <span style="color:green">Passed</span> <br>';
        } else {
            $o .= '<li class="remove">Delete Test DIR: <span style="color:red">Failed</span> <br>';
        }
    ...
    public function getLocationPrefs($location_id)
    {
        ...
        if (isset(self::$cachedUploadDest[$location_id])) {
            $location = self::$cachedUploadDest[$location_id];
        } else {
            $location = ee('Model')->get('UploadDestination')->filter('id', $location_id)->first();
            if ($location) {
                self::$cachedUploadDest[$location_id] = $location;
            }
        }
