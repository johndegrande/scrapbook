...
class ImageAction_watermark_text extends ImageAction
{
    ...
    public function run($file, $temp_dir)
    {
        ...
        if ($p['font_path'] == false && is_file($p['font_path']) == false) {
            if (is_file(APPPATH . 'fonts/texb.ttf') == true) {
                $p['font_path'] = APPPATH . 'fonts/texb.ttf';
            }
        }
    ...
    public function settings($settings)
    {
        ...
        if (isset($vData['horizontal_alignment']) == false) {
            $vData['horizontal_alignment'] = 'right';
        }
        if (isset($vData['vertical_alignment']) == false) {
            $vData['vertical_alignment'] = 'bottom';
        }
        ...
        if (isset($vData['text']) == false) {
            $vData['text'] = 'EEHarbor.Com (Channel Images)';
        }
        if (isset($vData['font_path']) == false) {
            $vData['font_path'] = '';
        }
        if (isset($vData['font_size']) == false) {
            $vData['font_size'] = '16';
        }
        if (isset($vData['font_color']) == false) {
            $vData['font_color'] = 'ffffff';
        }
        if (isset($vData['shadow_color']) == false) {
            $vData['shadow_color'] = '000000';
        }
        if (isset($vData['shadow_distance']) == false) {
            $vData['shadow_distance'] = '3';
        }
