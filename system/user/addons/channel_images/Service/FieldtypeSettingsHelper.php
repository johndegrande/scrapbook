    ...
    public static function saveSettings($field_id, $data)
    {
        ...
        if (isset($post['action_groups'])) {
            foreach ($post['action_groups'] as $order => $group) {
                ...
                $group['group_name'] = strtolower(ee('Format')->make('Text', $group['group_name'])->urlSlug());
    ...
    public static function settingsActions()
    {
        ...
        foreach (self::$settings['action_groups'] as &$group) {
            ...
            foreach ($actions as $action_name => &$settings) {
                ...
                if (isset($vdata['actions'][$action_name]) == false) {
                    continue;
                }
            ...
            if (isset($group['wysiwyg']) == true && $group['wysiwyg'] == 'no') {
                unset($group['wysiwyg']);
            }
            if (isset($group['editable']) == true && $group['editable'] == 'no') {
                unset($group['editable']);
            }
        }
        ...
        $fields[] = array(
            ...
            'fields' => array(
                ...
                'channel_images[columns_default][title]' => array(
                    ...
                    'attrs' => ' style="width:48%; float:right; margin-top: -47px;" ',
                ),
            ),
        );
