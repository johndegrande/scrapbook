...
use EEHarbor\ChannelImages\FluxCapacitor\Base\Mod;
 ...
 */
class Channel_images extends Mod
{
    public $LOCS = array();
    ...
    public function images()
    {
        ...
        if (ee()->TMPL->fetch_param('group_by_category') != false) {
            return $this->grouped_images(true);
        }
    ...
    public function images_static()
    {
        ...
        if (ee()->TMPL->fetch_param('category') != false) {
            ee()->db->where('category', ee()->TMPL->fetch_param('category'));
        }
    ...
    public function prev_next_image($which='next')
    {
        ...
        ee()->db->from('exp_channel_images');
        if (ee()->TMPL->fetch_param('image_id') != false) {
            ee()->db->where('image_id', ee()->TMPL->fetch_param('image_id'));
        }
        if (ee()->TMPL->fetch_param('url_title') != false) {
            ee()->db->where('url_title', ee()->TMPL->fetch_param('url_title'));
        }
        ee()->db->limit(1);
        $query = ee()->db->get();
    ...
    public function zip()
    {
        ...
        if (class_exists('CI_Location_local') == false) {
            require PATH_THIRD.'channel_images/locations/local/local.php';
        }
        ...
        foreach ($files as $file) {
            ...
            // Good Field?
            if (isset($fields[$field_id]) == false) {
                continue;
            }
    ...
    public function channel_images_router($mcp_task=false)
    {
        ...
        @header('Access-Control-Allow-Headers: Keep-Alive, Content-Type, User-Agent, Cache-Control, Origin, X-Requested-With, X-File-Name, X-File-Size, X-EEXID, X-CSRF-TOKEN');

        if (ee()->input->server('REQUEST_METHOD') == 'OPTIONS') {
            exit();
        }
        ...
        // Load Library
        if (class_exists('Channel_Images_AJAX') != true) {
            include 'ajax.channel_images.php';
        }
